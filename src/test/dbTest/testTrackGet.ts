...
type TagsCriteria = Extract<FilterSearchParam, { mode: "tags_any" | "tags_all" }>;
function tagsIn(tagIds: number[]): TagsCriteria {
    return {mode: "tags_all", tag_ids: tagIds.map(String)}
}
function tagsAny(tagIds: number[]): TagsCriteria {
    return {mode: "tags_any", tag_ids: tagIds.map(String)}
}
export async function initDb() {
    ...
    const allTracks = await getTracks(null, null, 100);
...
export async function Test1() {
    ...
    const assignTag = (assignement: TagAssignment, stamper: TagIdStamper) => {
        ...
        assignement.tag_ids.push(String(tagId));
        if (!tagAssignementsReversIdx.has(tagId)) {
            tagAssignementsReversIdx.set(tagId, new Set());
        }
        tagAssignementsReversIdx.get(tagId)!.add(Number(assignement.track_id));
    }
    ...
    const testTrackId = (tags: TagsCriteria, trackId: number) => {
        if (tags.mode === "tags_all") {
            return tags.tag_ids.every(t => tagAssignementsReversIdx.get(Number(t))?.has(trackId)) ?? false;
        }
        if (tags.mode === "tags_any") {
            return tags.tag_ids.some(t => tagAssignementsReversIdx.get(Number(t))?.has(trackId)) ?? false;
        }
        return false;
    }
    ...
    async function runTest(artistLike: number, trackLike: number, tagIds: TagsCriteria) {
        ...
        for (let i = 0; i < expected.length; i+= PAGE_STEP) {
            ...
            try {
                expect(tracks.map(t => Number(t.id))).toEqual(realExp);
            } catch (e) {
            ...
            cursor = String(expected[Math.min(i + PAGE_STEP, expected.length - 1)]);
